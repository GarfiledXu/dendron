---
id: mugbio5ueuzdmv6rassht1n
title: Tmp
desc: ''
updated: 1778125084670
created: 1778125070981
---
...
1. **运行态校验 (Status Check)**：基于当前设备真实环境、内部数据，验证是否满足执行条件。
2. **配置预校验 (Dry-Run)**：通过附带透传拟下发的配置负载，下位机进行合法性评估校验，返回校验结果。

### 1.2 价值体现

1. **校验逻辑集中维护**：应对固件差异，校验规则，划分与复杂判定收敛于下位机，上位机仅负责数据透传与结果显示，解决因硬编码导致的规则脱节问题。
...
```
不能说是两种模式，只能说分为两种校验内容，一种是不带配置负载则默认判断当前层级对象的内部状态，一种是带负载的，会额外校验内容，并且返回具体问题字段和错误
能否帮我优化一下这篇文档，我想补充的一些核心描述是，规则校验的转移，从上位机转移到下位机，从松散的交互到下位机固件绑定，这样可以实现很多效果，感觉目前的描述还不够简练
