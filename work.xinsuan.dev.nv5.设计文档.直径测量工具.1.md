...
tip: 鼠标左键按下后不释放，拖动roi，这个过程是会实时更新抽取圆的
...
2. 上位机UI截图:![alt text]({D8AB6C33-E794-489C-B15D-8F500BB86454}.png)
   ...
   2. 下位机行为:
   3. 算法行为:
3. 上位机UI截图:![alt text]({2B883C96-28C2-4441-B557-0EC1AD645EEB}.png)
   1. 上位机行为: roi覆盖被检测物体，会显示默认选定的直径边缘，以及直径箭头
   2. 下位机行为:
   3. 算法行为:
4. 上位机UI截图:![alt text]({3CA7A754-14C0-4A04-8DF7-65B703A6E2FD}.png)
   1. 上位机行为: 点击抽取直径，默认停留在标准灵敏度，roi区域会渲染所有的候选圆（黄色线条），以及当前选定圆（绿色+提示箭头指向），可以切换更大更小指定圆（若已经到极值，则对应按钮置灰）
   2. 下位机行为:
   3. 算法行为:
5. 上位机UI截图:![alt text]({05FD9EFA-3281-4DAC-A346-1FC84A2916B9}.png) ![alt text]({F55EF025-28DD-4F91-A2B9-04073FC2B094}.png)
   1. 上位机行为: 扩展功能抽取直径详细设定，切换直径模式，其中最大最小直径只会高亮对应唯一圆，指定直径和点击抽取直径面板相同, 明暗方向候选项
   2. 下位机行为:
   3. 算法行为:
6. 上位机UI截图:![alt text]({33174E55-79DA-46F9-B7F6-837D600A257F}.png)
   1. 上位机行为: 缩放设定，显示范围上限是显示值的3倍并只保留最高位，尾数全部抹零
   2. 下位机行为:
   3. 算法行为:
7. 上位机UI截图:![alt text]({99E8AB00-A61F-4C76-8FD3-2C7ABE5B2769}.png)
   1. 上位机行为: 阈值调节，注：上位机开发注意事项，下限不可拖动到上限右边
   2. 下位机行为:
   3. 算法行为:
8. 上位机UI截图:![alt text]({B3D2377B-A7B4-4B57-B1D7-2F23724C1A23}.png)
   1. 上位机行为: 运行界面
   2. 下位机行为:
   3. 算法行为:
